...
contract AdminFacet {
  ...
  function setPriceInfo(PriceInfo calldata priceInfo) external onlyOperator {
    ...
    PriceInfo storage current = bvs.priceInfos[priceInfo.productId];
    if (current.priceId == priceInfo.priceId && keccak256(bytes(current.symbol)) == keccak256(bytes(priceInfo.symbol))) {
      return;
    }
    ...
    if (oldPriceId != bytes32(0)) {
      oldSymbol = bvs.priceInfos[priceInfo.productId].symbol;
  ...
  /// @dev Idempotent: same productId + priceId as already stored is a no-op (no event). Otherwise upserts.
